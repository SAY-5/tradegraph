...
npm run selfcheck   # 64 assertions against the README numbers and the slice manifest
...
- the ontology. `src/data/slice.json` carries all 140 triples of
...
- the query semantics. `src/graph/queries.ts` follows the Spring Boot services hop for
  hop: the ultimate parent is found the way `exposure.rq` finds it, holders are the funds
...
| | slice | full sample |
|---|---:|---:|
| legal entities | 1,766 | 6,100 |
| positions | 4,092 | 12,373 |
| filings | 620 | 829 |
| lineage edges | 1,424 | 2,500 |
| triples | 65,550 | 207,095 |

`src/data/slice-manifest.json` records the counts and a sha256 of `slice.json`; the self
check recomputes both.

## What the numbers mean

The store in this page materialises the same triples the Python ETL writes and counts
them, so "65,550 triples" is a count, not a label. The four exposure pairs the README
...
src/graph/queries.ts       lineage, exposure, neighbours, search, trades, stats
...
src/data/slice.json        the committed slice (393 KiB)
